    ...
    get_friends();
    friend_requests();
    //add function to set email preference buttons to the correct value
}
    ...
    fetch("../apis/friend_requests.php", {
        ...
        .then(data => {
            ...
            for (i = 0; i < data.length; i++) {
                ...
                friend_request.style.cssText = "position: relative; padding-left: 12px; justify-content: start; align-items: start; gap: 7px; display: inline-flex;";
                ...
                friendName.innerHTML = '<div style="width: 77px; height: 25px; position: absolute; display: flex; justify-content: center; align-items: center; text-align: center; color: white; font-size: 12px; font-weight: bold; font-family: \'Inter\'; background-color: #498245;">' + data[i]["username"] + '</div>';
...
function change_email_preferences() {
    let email_preferences = document.getElementById("email_preferences").value;
    const formData = new FormData();
    formData.append("email_preferences", email_preferences);
    fetch("../apis/change_email_preferences.php", {
        method: "POST",
        body: formData
    })
        .then(response => response.text())
