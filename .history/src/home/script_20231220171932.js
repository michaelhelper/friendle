    ...
    fetch("../apis/user_data.php")
        ...
        .then(data => {
            ...
            if (lastWordleNumber == day) {
                ...
                wordle_stats.classList.add("h-[25px]", "mt-[12px]", "pl-[10px]", "pr-[10px]", "relative", "flex", "justify-center", "items-center", "text-center", "text-white", "text-md", "font-bold", "font-['Inter']", "bg-[#498245]", "mx-auto");
                let wordle = document.createElement("div");
                let wordle_data = data[data.length - 1]["wordle"];
                // delete all /r from the wordle
                wordle_data = wordle_data.replaceAll("\r", "");
                // split the wordle into an array of lines
                wordle_data = wordle_data.split("\n");
                // loop through the array of lines and create a div for each line
                for (let i = 0; i < wordle_data.length; i++) {
                    let line = document.createElement("div");
                    line.innerHTML = wordle_data[i];
                    line.classList.add("h-[25px]", "mt-[12px]", "pl-[10px]", "pr-[10px]", "relative", "flex", "justify-center", "items-center", "text-center", "text-white", "text-md", "font-bold", "font-['Inter']", "bg-[#498245]", "mx-auto");
