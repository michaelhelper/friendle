    ...
    fetch("../apis/friend_requests.php", {
        ...
        .then(data => {
            ...
            for (i = 0; i < data.length; i++) {
                ...
                friend_request.style.position = "absolute";
                friend_request.style.left = "7px";
                friend_request.style.justifyContent = "start";
                friend_request.style.alignItems = "start";
                friend_request.style.gap = "7px";
                friend_request.style.display = "inline-flex";
                friend_request.style.top = `${top + i * 35}px`;

                // create the inner elements
                let friendName = document.createElement("div");
                friendName.style.width = "77px";
                friendName.style.height = "25px";
                friendName.style.position = "relative";
                friendName.id = data[i]["username"];
                friendName.innerHTML = `<div id="${data[i]["username"]}3" style="width: 77px; height: 25px; left: 0; top: 0; position: absolute; display: flex; justify-content: center; align-items: center; text-align: center; color: white; font-size: 12px; font-weight: bold; font-family: 'Inter'; background-color: #498245;">${data[i]["username"]}</div>`;

                let acceptButton = document.createElement("button");
                acceptButton.style.width = "25px";
                acceptButton.style.height = "25px";
                acceptButton.style.position = "relative";
                acceptButton.id = data[i]["username"] + "4";
                acceptButton.innerHTML = `<button id="${data[i]["username"]}1" style="width: 25px; height: 25px; left: 0; top: 0; position: absolute; display: flex; justify-content: center; align-items: center; text-align: center; color: white; font-size: 12px; font-weight: bold; font-family: 'Inter'; background-color: #498245;" onclick="handle_friend_request('accept', this.id)">✓</button>`;

                let rejectButton = document.createElement("button");
                rejectButton.style.width = "25px";
                rejectButton.style.height = "25px";
                rejectButton.style.position = "relative";
                rejectButton.id = data[i]["username"] + "5";
                rejectButton.innerHTML = `<button id="${data[i]["username"]}2" style="width: 25px; height: 25px; left: 0; top: 0; position: absolute; display: flex; justify-content: center; align-items: center; text-align: center; color: white; font-size: 12px; font-weight: bold; font-family: 'Inter'; background-color: #AC9534;" onclick="handle_friend_request('deny', this.id)">x</button>`;
...
function handle_friend_request(action, friend) {
    ...
    // document.getElementById(friend + "1").remove();
    // document.getElementById(friend + "2").remove();
    // document.getElementById(friend + "3").remove();
    // document.getElementById(friend + "4").remove();
    // document.getElementById(friend + "5").remove();
    friend_requests();
}