        ...
        .then(data => {
            ...
            if (lastWordleNumber == day) {
                ...
                shareButton.onclick = function () {
                    ...
                    sharePopup.classList.add("fixed", "top-1/2", "left-1/2", "transform", "-translate-x-1/2", "-translate-y-1/2", "bg-white", "rounded-xl", "shadow-xl", "p-[10px]", "z-10");
                    // add the share statement
                    let shareStatement = document.createElement("div");
                    shareStatement.innerHTML = "Share your Wordle!";
                    shareStatement.classList.add("text-center", "font-bold", "text-lg", "mb-[10px]");
                    // add the wordle stats
                    let wordle_stats = document.createElement("div");
                    wordle_stats.innerHTML = "Wordle: " + data[data.length - 1]["wordle_number"] + " Score: " + data[data.length - 1]["wordle_score"] + "/6";
                    wordle_stats.classList.add("text-center", "font-bold", "text-lg", "mb-[10px]");
                    // add the wordle
                    let wordle = document.createElement("div");
                    wordle.classList.add("flex-col");
                    let wordle_data = data[data.length - 1]["wordle"];
