    ...
    fetch("../apis/friend_requests.php", {
        ...
        .then(data => {
            ...
            for (i = 0; i < data.length; i++) {
                ...
                friendName.innerHTML = `<div id="${data[i]["username"]}" class="w-[77px] h-[25px] left-0 top-0 absolute flex justify-center items-center text-center text-white text-xs font-bold font-['Inter'] bg-[#498245]">${data[i]["username"]}</div>`;
                let acceptButton = document.createElement("button");
                acceptButton.className = "w-[25px] h-[25px] relative";
                acceptButton.innerHTML = `<button class="w-[25px] h-[25px] left-0 top-0 absolute flex justify-center items-center text-center text-white text-xs font-bold font-['Inter'] bg-[#498245]" onclick="handle_friend_request('accept')">✓</button>`;
                
                let rejectButton = document.createElement("button");
                rejectButton.className = "w-[25px] h-[25px] relative";
                rejectButton.innerHTML = `<button class="w-[25px] h-[25px] left-0 top-0 absolute flex justify-center items-center text-center text-white text-xs font-bold font-['Inter'] bg-[#AC9534]" onclick="handle_friend_request('deny')">x</button>`;
