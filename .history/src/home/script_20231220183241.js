...
function Share_wordle(phonneNumber, wordleNumber, score, wordle) {
    ...
    navigator.clipboard.writeText(textToCopy)
        .then(() => {
            ...
            if (phonenNumber) {
                window.open(`sms:${phonenNumber}?body=${encodeURIComponent(textToCopy)}`);
            }
            const message = `Wordle 906 3/6\n\n⬛⬛⬛⬛⬛\n⬛🟩🟩🟩🟨\n🟩🟩🟩🟩🟩\n\nYour Turn: Friendle.com`;
            window.open(`sms:?body=${encodeURIComponent(message)}`);
            
            const phoneNumber = "9072809339";
