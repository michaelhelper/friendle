                ...
                fetch("../apis/get_friends.php", {
                    ...
                    .then(data => {
                        ...
                        for (i = 0; i < data.length; i++) {
                            ...
                            let wordleNumber = data[data.length - 1]["wordle_number"];
                            let wordleScore = data[data.length - 1]["wordle_score"];
                            let wordle = data[i]["wordles"][data.length - 1][wordle];
                            console.log(phoneNumber);
                            console.log(wordleNumber);
                            console.log(wordleScore);
                            console.log(wordle);
                            friend.onclick = function () {
                                Share_wordle(phoneNumber, wordleNumber, wordleScore, wordle);
                            }
...
function Share_wordle(phoneNumber, wordleNumber, score, wordle) {
    // Copy text to clipboard
    // const textToCopy = `Wordle 906 3/6\n\n⬛⬛⬛⬛⬛\n⬛🟩🟩🟩🟨\n🟩🟩🟩🟩🟩\n\nYour Turn: Friendle.com`;
    console.log(phoneNumber);
    console.log(wordleNumber);
    console.log(score);
    console.log(wordle);
    
    const textToCopy = "wordle " + wordleNumber + " " + score + "/6\n\n" + wordle + "\n\nYour Turn: Friendle.com";
    navigator.clipboard.writeText(textToCopy)
        .then(() => {
            // Open new text message with the text
            if (phoneNumber == 0) {
                window.open(`sms:?body=${encodeURIComponent(textToCopy)}`);
                
            }
            else {
                window.open(`sms:${phoneNumber}?body=${encodeURIComponent(textToCopy)}`);
            }
