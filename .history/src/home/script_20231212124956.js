    ...
    fetch("../apis/friend_requests.php", {
        ...
        .then(data => {
            ...
            for (i = 0; i < data.length; i++) {
                ...
                friendName.innerHTML = `<div id="${data[i]["username"]}3" class="w-[77px] h-[25px] left-0 top-0 absolute flex justify-center items-center text-center text-white text-xs font-bold font-['Inter'] bg-[#498245]">${data[i]["username"]}</div>`;
                
                let acceptButton = document.createElement("button");
                acceptButton.className = "w-[25px] h-[25px] relative";
                acceptButton.id = data[i]["username"] + "4";
                acceptButton.innerHTML = `<button id="${data[i]["username"]}1" class="w-[25px] h-[25px] left-0 top-0 absolute flex justify-center items-center text-center text-white text-xs font-bold font-['Inter'] bg-[#498245]" onclick="handle_friend_request('accept', this.id)">✓</button>`;
                
                let rejectButton = document.createElement("button");
                rejectButton.className = "w-[25px] h-[25px] relative";
                rejectButton.id = data[i]["username"] + "5";
                rejectButton.innerHTML = `<button id="${data[i]["username"]}2" class="w-[25px] h-[25px] left-0 top-0 absolute flex justify-center items-center text-center text-white text-xs font-bold font-['Inter'] bg-[#AC9534]" onclick="handle_friend_request('deny', this.id)">x</button>`;
