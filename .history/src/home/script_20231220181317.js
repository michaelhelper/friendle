        ...
        .then(data => {
            ...
            if (lastWordleNumber == day) {
                ...
                shareButton.onclick = function () {
                    //make a share popup on the screen with a list of friends to share with
                    let sharePopup = document.createElement("div");
                    sharePopup.classList.add("fixed", "top-1/2", "left-1/2", "transform", "-translate-x-1/2", "-translate-y-1/2", "bg-white", "w-[300px]", "h-[200px]", "rounded-xl", "shadow-xl", "flex-col", "justify-center", "items-center", "gap-1.5");
                    let shareStatement = document.createElement("div");
                    shareStatement.innerHTML = "Share with:";
                    shareStatement.classList.add("text-center", "text-md", "font-bold", "font-['Inter']");
                    let friendList = document.createElement("div");
                    friendList.classList.add("flex-col", "gap-1.5");
                    ...
                    fetch("../apis/get_friends.php", {
                        ...
                        .then(friends => {
                            ...
                            for (i = 0; i < friends.length; i++) {
                                ...
                                friend.classList.add("flex", "justify-center", "items-center", "gap-1.5");
                                friend.innerHTML = friends[i]["username"];
                                friend.onclick = function () {
                                    share_wordle(friends[i]["username"], data[data.length - 1]["wordle_number"], data[data.length - 1]["wordle_score"], data[data.length - 1]["wordle"]);
                                }
