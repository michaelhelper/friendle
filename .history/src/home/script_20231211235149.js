...
function add_wordle(){
    ...
    if (wordle[0] >= '0' && wordle[0] <= '9') {
        wordle_score = wordle[0];
    }
    //while wordle[0] is a number, letter, or space, continue adding to wordle
    while ((wordle[0] >= '0' && wordle[0] <= '9') || (wordle[0] >= 'a' && wordle[0] <= 'z') || (wordle[0] >= 'A' && wordle[0] <= 'Z') || wordle[0] == ' ') {
        wordle = wordle.substring(1);
    }
    alert (wordle_number + " " + wordle_score + " " + wordle);
