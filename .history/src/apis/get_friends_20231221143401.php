...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($result) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            // also check if the friend has accepted the friend request
            $query = "SELECT * FROM friends WHERE user_id = ? AND is_friend = 1";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("i", $row['friend_id']);
            $stmt->execute();
            $also_friend_result = $stmt->get_result();
            // if the result is empty, then the friend has not accepted the friend request exit
            if ($also_friend_result->num_rows == 0) {
                exit;
            }
