...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // check the length of the phone number it must be 10 digits
    if (strlen($phone) != ) {
        echo "<script type='text/javascript'>alert('Phone number must be 9 digits');</script>";
        exit;
    }
    ...
    if ($check_result->num_rows > 0) {
        echo "Username, email, or phone number already in use";
        exit;
    }
