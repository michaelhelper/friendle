...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($email) || empty($password)) {
        echo "<script type='text/javascript'>alert('Please fill out all fields";
        exit;
    }
    ...
    if ($result) {
        ...
        if ($conn->query($create_wordles_table_query) === TRUE) {
            echo "Table $wordles_table_name created successfully";
        } else {
        ...
        if ($conn->query($create_friends_table_query) === TRUE) {
            echo "Table $friends_table_name created successfully";
        } else {
            echo "Error creating table: " . $conn->error;
        }
